...
class EditorCanvas {
    ...
    render() {
        ...
        // Influence Overlay
        if (this.influenceData) {
            this.drawInfluence(w, h);
        }
    }
    ...
    setInfluence(level, influence) {
        this.influenceData = { level, data: influence };
        this.render();
    }

    drawInfluence(w, h) {
        if (!this.influenceData) return;
        
        const ctx = this.ctx;
        const { level, data } = this.influenceData;
        const stepSize = (level === 0) ? 16 : (level === 1) ? 4 : 1;
        
        const barH = 20; // Height of influence bar
        const y = h - barH;
        
        // Background
        ctx.fillStyle = 'rgba(20, 20, 25, 0.9)';
        ctx.fillRect(this.CONFIG.RULER_WIDTH, y, w - this.CONFIG.RULER_WIDTH, barH);
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const tEnd = (i + 1) * stepSize;
            
            const x1 = Math.floor(this.timeToX(tStart));
            const x2 = Math.ceil(this.timeToX(tEnd));
            
            if (x2 < this.CONFIG.RULER_WIDTH) continue;
            if (x1 > w) break;
            
            // Heatmap color: Cyan -> Magenta -> Red
            // Simple gradient
            const r = Math.floor(mag * 255);
            const g = Math.floor((1 - Math.abs(mag - 0.5) * 2) * 100); 
            const b = Math.floor((1 - mag) * 255);
            
            ctx.fillStyle = `rgba(${r}, ${g}, ${b}, 0.7)`;
            ctx.fillRect(Math.max(this.CONFIG.RULER_WIDTH, x1), y + 2, Math.max(1, x2 - x1 - 1), barH - 4);
        }
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'bottom';
        ctx.fillText(`COND L${level}`, this.CONFIG.RULER_WIDTH + 5, h - 4);
    }
}
