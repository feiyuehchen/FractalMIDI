...
from src.web.backend.config import get_config
from src.web.backend.model_manager import ModelManager, CheckpointInfo
from src.web.backend.example_manager import ExampleManager
from src.web.backend.inference_api import InferenceEngine
from src.web.backend.streaming_utils import WebSocketStreamer
from src.web.backend.td_bridge import td_bridge
from src.web.backend.osc_server import OSCBridge
...
# Mount static files
# Update static dir path to be relative to this file or root
# Assuming config.static_dir is set correctly in config.py or env
# If default was "web/frontend/dist", now it should be "code/web/frontend/dist"
# But config.py likely uses relative paths.
if config.static_dir.exists():
    app.mount("/static", StaticFiles(directory=str(config.static_dir)), name="static")
else:
    # Try relative to code/web
    static_fallback = Path("src/web/frontend/dist")
...
output_dir = Path("src/web/outputs")
...
class GenerationRequest(BaseModel):
    ...
    locked_channels: Optional[List[int]] = Field(None, description="List of channels to lock/keep (0=Density/Note, 1=Tempo/Vel)")
